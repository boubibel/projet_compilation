  ...
  | Vars (ids, opt_ty, init_seq) ->
      ...
      let ty =
        ...
        | Some t ->
            ...
            t
        | None ->
            (* On tente d'inférer le type à partir de la première affectation du seq *)
            ...
            | { idesc = Set (_, rhs); _ } :: _ ->
                ...
                | e1 :: rest ->
                    let t1 = type_expr env e1 in
                    (* on demande que tous les rhs aient le même type *)
                    List.iter
                      (fun e ->
                         let te = type_expr env e in
                         if te <> t1 then
                           error e.eloc "cannot infer unified type")
                      rest;
                    t1
                end
            | _ ->
                error loc "cannot infer type without initializer"
            end
      in
      (* ajout des variables dans l'environnement *)
      let env' =
        List.fold_left
          (fun acc x -> add_var loc x ty acc)
          env ids
      in
